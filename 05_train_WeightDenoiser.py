...
from model import GMANetPreTrain, WeightDenoiser, FlowMatchingDenoiser 
...
class FlowMatchingManager:
    ...
    @torch.no_grad()
    def sample(self, model, global_feat, local_feat, shape, init_vec=None, velocity_scale=1.0):
        ...
        for i in range(self.T):
            ...
            # Scale velocity back to original magnitude (model learned scaled velocity during training)
    ...
    for epoch in range(epochs):
        ...
        for target_vec, feat_l, feat_g in train_loader:
            ...
            if denoiser_type == "diff":
                ...
                t = beta_dist.sample((target_vec.shape[0],)).clamp(1e-4, 1.0 - 1e-4)
                t_view = t.view(-1, 1)
                z_t = (1.0 - t_view) * base_sample + t_view * target_vec
                z_t = torch.clamp(z_t, -5.0, 5.0)

                target_flow = (target_vec - base_sample) / global_vel_scale

                predicted_signal = model(z_t, t, feat_g, feat_l)
                base_loss = F.mse_loss(predicted_signal, target_flow)

                # x0 恢复时将速度缩回原始尺度
                pred_x0 = z_t + (1.0 - t_view) * predicted_signal * global_vel_scale
            ...
            # 总损失
            loss = noise_w * base_loss + w_w * w_loss
            loss.backward()
        ...
        logging.info(f"Epoch {epoch+1} | Loss: {avg_loss:.6f} | LR: {optimizer.param_groups[0]['lr']:.2e} | "
                     f"Phase: Base | "
                     f"Weights: noise={noise_w:.2f} W={w_w:.2f}")
# ==============================================================================
# 4. 测试与推理逻辑 (反归一化还原)
# ==============================================================================

def test_diffusion(model_path, stats_path, encoder, weights_dir, labeled_dir, device, local_num, global_num, result_path, diffusion_steps, denoiser_hidden_dim, denoiser_type, cond_input_dim=100, init_vec=None, velocity_scale=1.0):
    ...
    for subdir in test_subdirs:
        ...
        for f in tqdm(weight_files, desc=f"评估 {subdir.name}"):
            ...
            if denoiser_type == "diff":
                ...
                gen_vec_norm = flow_manager.sample(model, feat_g, feat_l, (1, W_DIM), init_vec=init_vec_norm, velocity_scale=velocity_scale)
...
if __name__ == "__main__":
    ...
    # Compute on CPU only to avoid device mismatch when estimating velocity scale.
    init_vec_norm = ((init_vec.detach().cpu().view(1, -1) - t_mean.view(1, -1)) / (t_std.view(1, -1) + 1e-8))
    all_velocities = []
    for target_vec, _, _ in raw_cache:
        velocity = target_vec.view(1, -1) - init_vec_norm
        all_velocities.append(velocity.norm(dim=-1).item())
    global_vel_scale = float(np.mean(all_velocities)) if all_velocities else 1.0
    global_vel_scale = max(global_vel_scale, 1e-6)
    ...
    train_diffusion(
        ...
        global_vel_scale=global_vel_scale,
    )
    ...
    test_diffusion(
        ...
        cond_input_dim=cond_input_dim,
        init_vec=init_vec,
        velocity_scale=global_vel_scale
    )